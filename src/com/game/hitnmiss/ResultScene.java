...
public class ResultScene extends CameraScene implements IOnSceneTouchListener {
	...
	float accureay = 1 - (float) scene._missCount / scene._bulletCount;
	if (Float.isNaN(accureay))
	    accureay = 0;
	accureay *= 100;
	Text result = new Text(0, 0, activity.gameFonts, activity.getString(R.string.accuracy) + ": " + String.format("%.2f", accureay)
